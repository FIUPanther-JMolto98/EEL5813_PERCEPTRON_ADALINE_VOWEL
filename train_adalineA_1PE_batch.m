% Trains the 'A - DETECTOR' using ADALINE learning, BATCH update, and 1 PROCESSING ELEMENT (PE) and returns its trained weights (W) and bias (b) when called using [W,b] = train_adalineA_1PE_batch(PP,TA,MAX_EPOCHS)

% FUNCTION DEFINITION
function [W,b,MSE_hist] = train_adalineA_1PE_batch(PP, TA, MAX_EPOCHS, ALPHA) % output the W (weights) and b (bias) variable so it remains in the MATLAB workspace and we can then pass it effortlessly to the training function
