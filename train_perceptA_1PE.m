% Trains the 'A - DETECTOR' using PERCEPTRON learning and 1 PROCESSING ELEMENT (PE) and returns its trained weights (W) and bias (b) when called using [W,b] = train_perceptA_1PE(PP,TA,MAX_EPOCHS)

% FUNCTION DEFINITION
function [W,b] = train_perceptA_1PE(PP, TA, MAX_EPOCHS) % output the W (weights) and b (bias) variable so it remains in the MATLAB workspace and we can then pass it effortlessly to the training function
